<div>
    ...
    <div class="w-full grid grid-cols-12 border bg-white text-black">
        @foreach ($users as $user )
            <div class="col-span-2 flex items-center text-center font-bold bg-white text-black">{{ $user->alias }}</div>
            <div class="col-span-9 bg-white text-black">
                <div class="flex flex-row gap-2 justify-between items-center border bg-white text-black">
                    @foreach ($user->picks->sortBy('game.game_date') as $pick)
                        <div class="col-span-1 gap-2">
                                {{-- Aqui es donde hay que desplegar los iconos y demas --}}
                                @if ($pick->game->allow_pick())
                                    <img src="{{ Storage::url($pick->game->visit_team->logo) }}"
                                    class="w-4 h-4 rounded-full border-solid  ">
                                @else
                                    <div class="col-span-1 flex flex-col justify-center items-center">
                                        <img src="{{ Storage::url($pick->game->visit_team->logo) }}"
                                            class="w-4 h-4 rounded-full border-solid  shadow-xl
                                            {{ $pick->winner == $pick->game->winner ? 'shadow-green-500' : 'grayscale' }} "
                                        >
                                        @if($pick->winner == $pick->game->winner)
                                            <img src="{{ asset('images/afirmativo.png') }}" class="w-2">
                                        @endif
                                    </div>
                                ...
                                {{-- Estos son los pronosticos antes --}}
                                {{-- @if ($pick->game->allow_pick())
                                    <img src="{{ asset('images/' . env('IMG_TO_YET_PICK_GAME', 'reloj.png')) }}"
                                        class="w-4 h-4 rounded-full border-solid border-black">
                                @else
                                    <div class="flex flex-col justify-center items-center border text-xxs sm:text-sm md:text-lg lg:text-2xl">
                                        <div class="flex justify-center items-center">
                                                <img src="{{ Storage::url($pick->winner == 1 ? $pick->game->local_team->logo : $pick->game->visit_team->logo) }}"
                                                    class="w-4 h-4 sm:w-16 rounded-full outline-double {{ $pick->winner == $pick->game->winner ? 'border-4 border-green-500 shadow-md' : 'border-4 border-red-500 shadow-md' }}"
                                                >
                                        </div>
                                        @if($pick->winner == $pick->game->winner)
            ...
            <div class="col-span-1 flex items-center text-center font-bold justify-center ml-2 bg-white text-black">
                {{ $user->positions->first() ? $user->positions->first()->hits : 'X'}}
            </div>
